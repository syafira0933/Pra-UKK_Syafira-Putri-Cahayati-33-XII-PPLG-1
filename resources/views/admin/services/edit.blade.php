<x-admin-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-900 leading-tight">Edit Layanan</h2>
    </x-slot>
            ...
            <div class="bg-white p-6 sm:p-8 border border-krem-dark/40 shadow-sm rounded-3xl space-y-6">
                ...
                <form method="POST" action="{{ route('admin.services.update', $service) }}" enctype="multipart/form-data" class="space-y-5">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $service->name) }}" class="w-full border-krem-dark/60 rounded-xl text-sm focus:border-coklat focus:ring-coklat bg-krem-light/30 p-3">
                    </div>

                    <div>
                        <label class="block font-semibold text-xs text-gray-700 mb-1.5">Deskripsi</label>
                        <textarea name="description" rows="3" class="w-full border-krem-dark/60 rounded-xl text-sm focus:border-coklat focus:ring-coklat bg-krem-light/30 p-3">{{ old('description', $service->description) }}</textarea>
                    </div>

                    @if ($service->image)
                        <div>
                            <img src="{{ Storage::url($service->image) }}" class="w-24 h-24 object-cover rounded-2xl border border-krem-dark/30 shadow-xs">
                        </div>
                    @endif

                    <div>
                        <label class="block font-semibold text-xs text-gray-700 mb-1.5">Ganti Gambar (opsional)</label>
